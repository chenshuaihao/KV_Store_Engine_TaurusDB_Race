...
size_t KeyFile::SetKey(KVString & key) {
    //写入key
...
int64_t KeyFile::GetKey(int64_t pos, KVString & key) {
    char *p = new char[8];
    memcpy(p, (keyCacheBuffer_ + (pos << 3)), 8); //*(keyCacheBuffer_ + keyFilePosition_ << 3) = *((size_t *) key.Buf()); 
    key.Reset(p, 8);
    return 0;
}