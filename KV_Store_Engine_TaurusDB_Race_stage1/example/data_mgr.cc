...
int DataMgr::kMaxFileSize = MAX_FILE_SIZE; //default 256MB per file

int DataMgr::Init(const char * dir, int file_size) {
    Release();
...
uint64_t DataMgr::Append(KVString & key, KVString & val) {
    ...
    if (cur_file_size_ > kMaxFileSize) {
        NewFile();
    }
...
void DataMgr::NewFile() {
    ...
    files_info_[cur_file_no_] = std::unique_ptr<FileIo>(
        new FileIo(No2FileName(cur_file_no_, ".data").Buf()));
    cur_file_size_ = 0;
}
